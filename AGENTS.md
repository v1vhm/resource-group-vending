...
* **File names:** Environment files are named `${product_short_name}_${environment}.yaml`.  The workflow uses the short name and environment tier to build this file name and the resource names.  Do not include spaces or uppercase letters in file names.  The workflow also uses `${product_short_name}_${environment}_${location}` for the state container name and YAML file inside the job, so ensure the values are valid for Azure storage container naming (lowercase alphanumeric and hyphens; length ≤ 63 characters).
...
1. Run the **Provision Environment** workflow manually via the GitHub Actions UI or allow Port to trigger it.  Provide appropriate values for all inputs.  Ensure that `product_short_name` meets the 6–10 character constraint.  The workflow will create a YAML file under `environments/`, create the state container, run Terraform and commit the YAML file and its updates.
