...
| `terraform/modules/resource_group/` | Module that creates the Azure resource group, storage account and user‑managed identity.  It names the resource group using the pattern `rg-${var.product_identifier}-${var.environment}-${var.location}` and storage accounts as `v1vhm${var.product_identifier}${var.environment}${var.location}` to ensure global uniqueness.  It sets tags based on the YAML properties, creates a federated credential for GitHub Actions and outputs resource attributes for the root configuration.  Port entities are registered in the root, not within this module. |
