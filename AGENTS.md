...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.github/workflows/provision.yml`   | GitHub Actions workflow that provisions environments.  It accepts inputs such as `product_name`, `product_short_name`, `location`, `environment`, `product_identifier` and a JSON‑encoded `port_context`.  The workflow creates a YAML file under `environments/`, commits it, creates a provisioning state container (`vending_state_container`), runs Terraform `init`/`plan`/`apply` and appends Terraform outputs back into the YAML file. Services can be associated later via a separate workflow. |
| `environments/`                     | Contains one YAML file per environment.  These files follow the schema described in the README: `environment_identifier`, `environment_title`, `location`, `environment` (dev/test/prod/acct), `port_run_id`, `product_name`, `product_identifier`, `status` (records `in_progress`, `succeeded` or `failed`), the provisioning container `vending_state_container` and an optional `services` list with entries `service_identifier`, `deployment_state_container` and `github.repository` for each service.  Terraform creates a deployment state container and two GitHub OIDC federated credentials (`<environment_identifier>-plan` and `<environment_identifier>-apply`) for every listed service.  Workflows and Terraform read these files to provision resources and attach services. |
| `terraform/`                        | Root Terraform configuration.  It uses a local variable to read all YAML files in the `environments` folder and creates a `module` instance for each environment.  It processes any services listed for association, creating a storage container and two GitHub OIDC federated credentials for each and upserting a Port `azureStorageContainer` entity, then outputs deployment details. |
| `terraform/modules/resource_group/` | Module that creates the Azure resource group, storage account, user‑managed identity, and per‑service deployment state containers with GitHub OIDC federated credentials.  It names the resource group using the pattern `rg-${var.product_identifier}-${var.environment}-${var.location}` and storage accounts as `v1vhm${var.product_identifier}${var.environment}${var.location}` to ensure global uniqueness.  It sets tags based on the YAML properties and outputs resource attributes for the root configuration.  Port entities are registered in the root, not within this module. |

## Naming conventions and data contracts

* **YAML schema:** The environment YAML schema in the README shows the expected keys and constraints, including `environment_identifier`, `environment_title` and a required `status` field. `status` must be `in_progress`, `succeeded` or `failed` and records the provisioning lifecycle. `environment` must be one of `dev`, `test`, `prod` or `acct`, and each file includes `product_name`, `product_identifier` and the provisioning container `vending_state_container`.  The optional `services` list uses entries like:

  ```yaml
    vending_state_container: envstate-demo-dev-eastus
    services:
  ...
  The provisioning workflow creates `vending_state_container` in the shared `vendingtfstate` storage account using the name `envstate-<product_short_name>-<environment>-<location>` before running Terraform. Terraform creates a deployment state container in the environment's storage account for each `service_identifier`, records the container name under `deployment_state_container` and configures two GitHub OIDC federated credentials named `<environment_identifier>-plan` and `<environment_identifier>-apply` for that service.  When creating new environment files, adhere to this schema.
* **File names:** Environment files are named `${product_short_name}_${environment}_${location}.yaml`.  The workflow uses the short name and environment tier to build this file name and related resource names.  Do not include spaces or uppercase letters in file names.  The provisioning state container is named `envstate-${product_short_name}-${environment}-${location}`.  Deployment state containers use each `service_identifier` as the container name.
...
4. **Provisioning state container:** The workflow creates an Azure Storage container in the shared `vendingtfstate` account named `envstate-${product_short_name}-${environment}-${location}`.  If you change the naming scheme, update `backend.tf` and ensure containers are created consistently.  Do not hardcode secrets; use repository secrets for credentials.
...
* **Outputs:** The resource group module exposes detailed attributes for the
  resource group, storage account, deployment state containers (and their federated
  ...
  each deployment state container, and exposes high‑level values
...
1. Run the **Provision Environment** workflow manually via the GitHub Actions UI or allow Port to trigger it.  Provide appropriate values for all inputs.  The workflow will create a YAML file under `environments/`, create the provisioning state container (`vending_state_container`), run Terraform and commit the YAML file and its updates.
...
3. If services need to be associated, run the dedicated service‑association workflow to populate the `services` list.  Terraform then provisions a storage container and two GitHub OIDC federated credentials for each service and registers a corresponding `azureStorageContainer` entity in Port.
4. Inspect the Azure portal to confirm that the resource group, storage account, deployment state containers and user‑managed identity have been created with the expected names.  Ensure tags reflect the environment metadata.
